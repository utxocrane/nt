...
const {updateGiteeFile,createRetryAxios}=require('./lib/common')
...
async function getBocData(){
	...
	for(let si=0;si<=maxStartIndex;si+=pageSize){
		...
		let res = await a.post(process.env.BOCLSTURL,'json='+encodeURI(JSON.stringify(reqbody)))
	...
	for(let v of invList){
		...
		let res = await a.post(process.env.BOCDTLURL,'json='+encodeURI(JSON.stringify(reqbody)))
...
async function getAll(){
	...
	let sdata=JSON.stringify(invTab)
	fs.writeFileSync(dataPath,sdata,'utf-8')
	fs.writeFileSync(orgDataPath,JSON.stringify(originData),'utf-8')
	updateGiteeFile(process.env.GITEETOKEN,process.env.GITEEURLBKNV,sdata)
}
