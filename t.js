...
async function updateData() {
	//console.log(await getUploadsPlaylistId('https://www.youtube.com/@blue-Youtube/videos'))
	let allTxt = '',allLogs=''
	/////////////////普通base64订阅链接或包含节点URL的纯文本，按是否包含:判断是否需要base64解码
	const suburls=[]
	
	suburls.push(...(await loadYoutubeSbj('UUtzk4Wh7dwJLDKXbq4w4PRQ',/https:\/\/us1\.zhuk\.dpdns\.org\/[^/\s]+\outube.html/g , /https:\/\/us1\.zhuk\.dpdns\.org\/\S+?\.txt/g)))
	...
	suburls.push(...(await loadShareSite())) //分享站点
...
async function getUploadsPlaylistId(channelUrl) {
    // 
    const handle = new URL(channelUrl).pathname.split('/')[1]

    if (!handle.startsWith('@')) {
        throw new Error('不是 YouTube handle 链接')
    }

    // 通过 handle 获取 channelId 和 uploads playlistId
...
async function loadYoutubeSbj(playlistid,rgx,rgx2){
	...
	try{
	let matches=[]
	for(let item of (await axios.get('https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId='+playlistid+'&maxResults=50&key='+process.env.YTKEY,{httpsAgent:socksAgent})).data.items){
		const ms = item.snippet.description.match(rgx)
		if(!ms) continue
		matches = ms
		break
	}
	console.log('找到YT视频简介里提出主链接',matches)
